...
// this type cannot be thread-safe
...
impl<T> RefCell<T> {
    ...
    // The following must be Options because of the rust contract that:
    // If there is a shared reference there is no exclusive ref and vice versa
    pub fn borrow(&self) -> Option<Ref<'_, T>> {
        match self.state.get() {
            RefState::Unshared => {
                self.state.set(RefState::Shared(1));
                Some(Ref { refcell: self })
                // SAFETY: no exclusive references have been given out. Otherwise state would have been set to Exclusive
    ...
    pub fn borrow_mut(&self) -> Option<RefMut<'_, T>> {
        if let RefState::Unshared = self.state.get() {
            ...
            // SAFETY: no other references have been given out because then state would be shared or exclusive
...
impl<T> std::ops::Deref for Ref<'_, T> {
    // Deref is the trait that gets called when you use the .method
    // follow deeper into a type; deref into the inner type T
    type Target = T;
...
impl<T> std::ops::DerefMut for RefMut<'_, T> {
    fn deref_mut(&mut self) -> &mut Self::Target {
        // SAFETY:
        // a RefMut is only created if no other references haver been given out.
